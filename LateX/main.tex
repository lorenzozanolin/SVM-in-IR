...
\begin{frame}{K-means}
    \only<1->{The aim is to create clusters that contain \textit{similar documents}. Given a training set ${x^{(1)},x^{(2)},\ldots,x^{(m)}}$, the algorithm used is the following:\\}
    \uncover<2->{
        \begin{algorithm}[H]\small
            ...
            \begin{algorithmic}[1]
              ...
              \Procedure{K-Means}{$K$}
                \State \texttt{initialize cluster centroids $\mu_1,\ldots,\mu_k$ ramdomly.} 
...
\begin{frame}{Hybrid IR System}
    \only<1->{The proposed system can be summarized by the following figure:}
    \uncover<2->{
        \begin{figure}[]
...
\begin{frame}{Queries classification}
    ...
    \vspace{3.5mm}
    \uncover<4->{So, this process allows the selection of documents from the returned class to return first. \cite{article1}}
